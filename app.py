...
# Avoid runtime pip installs unless explicitly allowed.
...
# Prefer token from env to avoid interactive prompts in non-TTY contexts.
...
def _select_device_and_dtype():
    ...
    forced_device = os.environ.get("QWEN_DEVICE")
    forced_dtype = os.environ.get("QWEN_DTYPE")
    if forced_device:
        dtype = _parse_dtype(forced_dtype) if forced_dtype else None
        return forced_device, dtype

    # Auto-detect CUDA; fall back to CPU for missing/unsupported CUDA libs.
    try:
...
@spaces.GPU(duration=60)
def save_voice_clone_prompt(ref_audio, ref_text, use_xvector_only, model_size):
    """Create and save a reusable voice clone prompt (.pt)."""
    audio_tuple = _audio_to_tuple(ref_audio)
    if audio_tuple is None:
        return None, "Error: Reference audio is required."
    ...
    try:
        ...
        return out_path, f"Saved reusable prompt: {out_path}"
    except Exception as e:
        return None, f"Error: {type(e).__name__}: {e}"
        ...
        with gr.Tabs():
            ...
            with gr.Tab("Voice Clone (Base)"):
                ...
                clone_save_btn.click(
                    ...
                    inputs=[clone_ref_audio, clone_ref_text, clone_xvector, clone_model_size],
                    outputs=[clone_prompt_file_out, clone_status],
                )
